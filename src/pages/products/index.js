    ...
    <div className={`bg-gray-100 ${styles.backgroundImage}`}>
      <div className={`grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 p-4 ${styles.productGrid}`}>
        {products.map((product) => (
          <div key={product.id} className={`bg-white rounded-lg shadow-md p-4 ${styles.productCard}`}>
            <div onClick={() => openModal(product.id)} className={styles.productCardInner}>
      ...
      {selectedProduct && (
        <div className={styles.modal} onClick={closeModal}>
          <div className={styles.modalContent} onClick={(e) => e.stopPropagation()}>
            ...
            {/* Display product details for the selected product here */}
            {/* Example: */}
            <h2>Product Details</h2>
            <img src={products[selectedProduct - 1].image} alt={`Product ${selectedProduct}`} />
            <p>Name: {products[selectedProduct - 1].name}</p>
            <p>Description: {products[selectedProduct - 1].description}</p>
            <p>Price: ${products[selectedProduct - 1].price}</p>
